...
def client():
    ...
    value.create.return_value = {
        ...
        "id": "123456789012",
        "url": "https://jules.google.com/task/123456789012",
    }
    return value
...
def packet():
    return {
        "effect": "write",
        "execution": {"owner_repository": "owner/repo", "exact_base": HEAD, "topic_branch": "fix/example"},
        "authority": {"path_prefixes": ["src/"]},
        "intent": {"intended_effect": "repair verified defect"},
        "predicate": "python -m unittest",
    }
...
class AdapterTests(unittest.TestCase):
    ...
    def test_api_launch_uses_branch_marker_and_no_auto_pr(self):
        ...
        with (
            patch("limen.jules_api_adapter._default_branch", return_value="trunk"),
    ...
    def test_changed_source_head_stops_before_create(self):
        ...
        with (
            ...
            patch("limen.jules_api_adapter.remote_branch_head", return_value="b" * 40),
        ):
            with self.assertRaises(FanoutExecutionError):
    ...
    def test_no_free_slot_stops_before_create(self):
        ...
        remote.sessions.return_value = Catalog(
            tuple({"name": f"sessions/{i}", "state": "PAUSED", "createTime": now} for i in range(15)), 1, ""
        )
        with self.assertRaises(FanoutExecutionError):
    ...
    def test_roll_limit_stops_before_create(self):
        ...
        now = datetime.now(timezone.utc).isoformat()
        remote.sessions.return_value = Catalog(
            tuple({"name": f"sessions/{i}", "state": "COMPLETED", "createTime": now} for i in range(100)), 1, ""
        )
        with self.assertRaises(FanoutExecutionError):
            JulesApiExecutionAdapter(remote).launch(packet(), "attempt-abc-1")
        remote.create.assert_not_called()
    ...
    def test_landing_patch_is_bound_to_source_and_head(self):
        ...
        adapter = JulesApiExecutionAdapter(remote)
        with patch(
            "limen.jules_api_adapter._checked", side_effect=["https://github.com/owner/repo.git", HEAD, ""]
        ) as command:
