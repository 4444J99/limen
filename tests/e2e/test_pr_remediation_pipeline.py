"""Synthetic unit models for PR-remediation design examples.

These tests exercise local stand-in engines and do not import production
remediation code. Passing results describe the models only; they cannot certify
live CI, a merged PR, safe deletion, custody, or preserved running processes.
Production acceptance uses the owning exact-head predicates and remote receipts.
"""
...
def test_tier1_f1_03_active_vs_dormant_repo_segmentation():
    ...
    report = engine.generate_telemetry_report()
    assert report["active_repositories"] == 247
    # 247 core active repositories out of 319 total estate
    estate_total = 319
...
def test_tier1_f2_01_hermetic_runner_environment_isolation(monkeypatch):
    ...
    # Verify isolation fixture resets state after run
...
def test_tier1_f3_03_fixable_ci_stale_fixture_remediation(tmp_path):
    ...
    # Simulate repairing a fixture file
...
def test_tier1_f5_02_dependency_bound_remediation():
    ...
    # Remediate ruff pin to match repo standard (ruff==0.15.8)
    deps["ruff"] = "==0.15.8"
...
def test_tier2_f1_03_chamber_with_zero_open_prs_handled():
    ...
    report = engine.generate_telemetry_report()
    # At least some chambers will have 0 PRs
...
def test_tier2_f3_05_circular_symlink_in_worktree_ignored(tmp_path):
    ...
    try:
        ...
        pass  # On systems without symlink privilege
    # Directory walk with follow_symlinks=False is safe
...
def test_tier3_06_batch_remediation_with_partial_failures_and_receipt_logging(tmp_path):
    ...
    for i in range(5):
        ...
        fail = i == 2  # Injected failure for PR #602
...
def test_tier4_05_idempotent_fixed_point_full_estate_re_execution(tmp_path):
    ...
    # Hash should match if generated at same head
...
def test_tier5_01_adversarial_malformed_receipt_injection_and_tampering(tmp_path):
    ...
    # Hash verification detects tamper
