#!/usr/bin/env python3
"""Local Git fixture checks and synthetic PR-remediation contract examples.

The model checks in this module do not prove production CI, repository landing,
remote branch custody, or preservation of other sessions. Git rebase checks use
real objects in an isolated temporary clone. Live preservation requires paired,
owner-provided observations; absence of those observations remains unverified.
"""
...
class EcosystemVerifier:
    ...
    def verify_clean_rebase(
        ...
        """Run a real rebase in a disposable clone; never manufacture Git objects."""
        commits = [base_commit, *pr_commits, *upstream_commits]
        if not pr_commits or any(
            not isinstance(sha, str) or len(sha) != 40 or any(char not in "0123456789abcdef" for char in sha)
            for sha in commits
        ):
            return {"success": False, "error": "Full Git commit identities are required"}
        if has_conflicts:
            return {"success": False, "error": "Known conflicts require reviewed resolution"}
        environment = dict(os.environ)
        environment.update(
            {
                "GIT_CONFIG_GLOBAL": os.devnull,
                "GIT_CONFIG_NOSYSTEM": "1",
                "GIT_AUTHOR_NAME": "Fixture verifier",
                "GIT_AUTHOR_EMAIL": "fixture@example.invalid",
                "GIT_COMMITTER_NAME": "Fixture verifier",
        ...
        try:
            for sha in commits:
                if git(self.limen_root, "cat-file", "-e", sha + "^{commit}").returncode:
                    return {"success": False, "error": "A supplied commit is unavailable"}
            for tip, expected in [
                (pr_commits[-1], pr_commits),
                (upstream_commits[-1], upstream_commits) if upstream_commits else (base_commit, []),
            ]:
                if git(self.limen_root, "merge-base", "--is-ancestor", base_commit, tip).returncode:
                    return {"success": False, "error": "Commit lineage does not contain the base"}
                actual = git(self.limen_root, "rev-list", "--reverse", base_commit + ".." + tip)
                if actual.returncode or actual.stdout.splitlines() != expected:
                    return {"success": False, "error": "Supplied commits do not cover the complete lineage"}
            upstream = upstream_commits[-1] if upstream_commits else base_commit
            with tempfile.TemporaryDirectory(prefix="limen-rebase-fixture-") as temporary:
                clone = git(None, "clone", "--shared", "--no-checkout", "--quiet", str(self.limen_root), temporary)
                if clone.returncode:
                    return {"success": False, "error": "Isolated clone failed"}
                checkout = git(temporary, "checkout", "--detach", pr_commits[-1])
                if checkout.returncode:
                    return {"success": False, "error": "Isolated checkout failed"}
                rebase = git(temporary, "rebase", "--no-gpg-sign", "--onto", upstream, base_commit)
                if rebase.returncode:
                    return {"success": False, "error": "Git could not rebase without reviewed conflict resolution"}
                head = git(temporary, "rev-parse", "HEAD").stdout.strip()
                if git(temporary, "cat-file", "-e", head + "^{commit}").returncode:
                    return {"success": False, "error": "Resulting commit is unavailable"}
                return {
                    "success": True,
                    "rebased_head": head,
                    "rebase_type": "clean-rebase" if upstream_commits else "clean-fast-forward",
                    "commit_count": len(pr_commits),
                    "evidence_scope": "isolated_git_fixture",
                }
        except (OSError, subprocess.TimeoutExpired):
            return {"success": False, "error": "Git verification unavailable or timed out"}
    ...
    def verify_three_way_merge_resolution(
        ...
        """Use Git's merge algorithm, preserving order and refusing unresolved conflicts."""
        with tempfile.TemporaryDirectory(prefix="limen-merge-fixture-") as temporary:
            paths = [Path(temporary) / name for name in ("upstream", "base", "pr")]
            for path, content in zip(paths, (upstream_content, base_content, pr_content)):
                path.write_text(content, encoding="utf-8")
            try:
                result = subprocess.run(
                    ["git", "merge-file", "-p", *map(str, paths)], capture_output=True, text=True, timeout=10
                )
            except (OSError, subprocess.TimeoutExpired):
                return False, "Merge verification unavailable"
    ...
    def verify_force_with_lease_safety(
        ...
        if expected_remote_sha != actual_remote_sha:
            return (
                False,
                f"Force-with-lease rejected push: remote head {actual_remote_sha} != expected {expected_remote_sha}",
            )
    ...
    def verify_github_merge_execution(
        ...
        return {
            "success": True,
            "evidence_scope": "command_shape_only",
            "executed": False,
            "pr_number": pr_number,
            "strategy": strategy,
    ...
    def verify_worktree_lifecycle_and_reaping(
        ...
        if worktree_path.exists():
            return False, "Path is still present; verification never deletes it"
        return True, "Fixture path absent; no repository custody or landing claim"
    ...
    def verify_live_invariants(self) -> Dict[str, Any]:
        """Report missing preservation evidence without probing unrelated private owners."""
        return {
            "evidence_scope": "unverified_live_preservation",
            "private_document_review_untouched": None,
            "role_hardening_branch_present": None,
            "universal_mail_untouched": None,
            "daemons_active": None,
            "verified": False,
            "violations": [],
            "missing_evidence": [
                "owner-qualified before and after repository identities",
                "exact refs and custody receipts",
                "before and after process identities",
            ],
        }
...
@contextmanager
def _git_fixture(verifier, *, conflict=False):
    """Create real diverging history without touching the user's repository."""
    prior = verifier.limen_root
    with tempfile.TemporaryDirectory(prefix="limen-history-fixture-") as temporary:
        root = Path(temporary)
        environment = dict(os.environ)
        ...
        git("checkout", "-b", "pr")
        (root / ("base.txt" if conflict else "feature.txt")).write_text("feature\n")
        git("add", ".")
        git("commit", "-m", "feature")
        pr = git("rev-parse", "HEAD")
        git("checkout", "main")
        (root / ("base.txt" if conflict else "upstream.txt")).write_text("upstream\n")
        git("add", ".")
        git("commit", "-m", "upstream")
        upstream = git("rev-parse", "HEAD")
        verifier.limen_root = root
        try:
            yield base, [pr], [upstream]
        finally:
            verifier.limen_root = prior


def test_tier1_r1_clean_rebase_against_default_branch(verifier):
    with _git_fixture(verifier) as (base, commits, upstream):
        result = verifier.verify_clean_rebase(base, commits, upstream)
        assert result["success"] is True
        assert result["evidence_scope"] == "isolated_git_fixture"
        assert result["rebase_type"] == "clean-rebase"
        assert result["commit_count"] == 1


def test_tier1_r1_three_way_merge_conflict_resolution(verifier):
    base = "line1\nline2\nline3\nline4\nline5\n"
    upstream = base.replace("line1", "upstream")
    pr = base.replace("line5", "feature")
    ok, merged = verifier.verify_three_way_merge_resolution(base, upstream, pr)
    assert ok is True
    assert merged == "upstream\nline2\nline3\nline4\nfeature\n"
    ok, _ = verifier.verify_three_way_merge_resolution("base\n", "left\n", "right\n")
    assert ok is False


def test_tier1_r1_detached_head_handling(verifier):
    assert verifier.verify_clean_rebase("NOT-A-COMMIT", ["ALSO-INVALID"], ["MISSING"])["success"] is False
    with _git_fixture(verifier, conflict=True) as (base, commits, upstream):
        assert verifier.verify_clean_rebase(base, commits, upstream)["success"] is False
        assert verifier.verify_clean_rebase(base, commits, upstream, has_conflicts=True)["success"] is False


def test_tier1_r1_fast_forward_mergeability(verifier):
    with _git_fixture(verifier) as (base, commits, _):
        result = verifier.verify_clean_rebase(base, commits, [])
        assert result["success"] is True
        assert result["rebase_type"] == "clean-fast-forward"
...
def test_tier2_r2_local_test_suite_execution(verifier):
    ...
    fail_ok, fail_msg = verifier.verify_local_test_suite_execution(
        "pytest tests/", 1, "FAILED test_a.py - AssertionError"
    )
...
def test_tier3_r3_exact_head_commit_matching(verifier):
    ...
    stale_ok, stale_msg = verifier.verify_exact_head_merge_policy(
        verified_sha, "drifted_head_sha_9999999999999999999999"
    )
...
def test_tier4_r4_worktree_lifecycle_and_reaping(tmp_path, verifier):
    ...
    ok, _ = verifier.verify_worktree_lifecycle_and_reaping(wt_path, exclude_file)
    assert ok is False
    assert wt_path.exists()
    wt_path.rmdir()  # Only the fixture removes the empty directory it created.
    ok, _ = verifier.verify_worktree_lifecycle_and_reaping(wt_path, exclude_file)
    assert ok is True
...
def test_tier4_r4_zero_unceremonious_closures(verifier):
    ...
    records = [
        ...
        PRRemediationRecord(
            102, "organvm/domus", "organvm", "feat/2", is_sensitive=True, disposition="LEAVE_OPEN_WITH_DIAGNOSTIC"
        ),
...
def test_invariant_r5_private_document_review_untouched(verifier):
    ...
    allowed, msg = verifier.verify_protected_path_guard(
        "/Users/4jp/Workspace/limen-private-document-review-20260825/file.txt"
    )
...
def test_e2e_full_ecosystem_verification_cycle(verifier):
    ...
    assert live_invariants["private_document_review_untouched"] is None
    assert live_invariants["daemons_active"] is None
    assert live_invariants["verified"] is False
    assert live_invariants["missing_evidence"]
...
def run_standalone_harness(args: argparse.Namespace) -> int:
    ...
    print("=" * 80)
    print("LIMEN LOCAL GIT FIXTURE AND REMEDIATION MODEL CHECKS")
    ...
    def execute_temporary(test):
        with tempfile.TemporaryDirectory(prefix="limen-model-fixture-") as temporary:
            return test(Path(temporary), verifier)
    ...
    if args.tier in ("1", "all"):
        ...
        execute_test(
            ...
            "R1",
            "Tier 1",
            "Clean Rebase Against Default Branch",
            lambda: test_tier1_r1_clean_rebase_against_default_branch(verifier),
        )
        execute_test(
            "T1.2",
            "R1",
            ...
            lambda: test_tier1_r1_three_way_merge_conflict_resolution(verifier),
        )
        execute_test(
            "T1.3",
            "R1",
            ...
            lambda: test_tier1_r1_detached_head_handling(verifier),
        )
        execute_test(
            "T1.4",
            "R1",
            "Tier 1",
            "Fast-Forward Mergeability",
            lambda: test_tier1_r1_fast_forward_mergeability(verifier),
        )
        execute_test(
            "T1.5",
            "R1",
            "Tier 1",
            "Commit Provenance Preservation",
            lambda: test_tier1_r1_commit_provenance_preservation(verifier),
        )
    ...
    if args.tier in ("2", "all"):
        ...
        execute_test(
            ...
            "R2",
            "Tier 2",
            "Local Test Suite Execution Verification",
            lambda: test_tier2_r2_local_test_suite_execution(verifier),
        )
        execute_test(
            "T2.2",
            "R2",
            "Tier 2",
            "Syntax, Lint, and Typecheck Gates",
            lambda: test_tier2_r2_syntax_lint_typecheck_gates(verifier),
        )
        execute_test(
            "T2.3",
            "R2",
            "Tier 2",
            "CI Workflow Diagnostics & Repair",
            lambda: test_tier2_r2_ci_workflow_diagnostics_and_repair(verifier),
        )
        execute_test(
            "T2.4",
            "R2",
            ...
            lambda: test_tier2_r2_timeout_and_resource_bounds(verifier),
        )
        execute_test(
            "T2.5",
            "R2",
            ...
            lambda: execute_temporary(test_tier2_r2_hermetic_isolation),
        )
    ...
    if args.tier in ("3", "all"):
        ...
        execute_test(
            ...
            "R3",
            "Tier 3",
            "Force-with-Lease Safety Guard",
            lambda: test_tier3_r3_force_with_lease_safety_guard(verifier),
        )
        execute_test(
            "T3.2",
            "R3",
            "Tier 3",
            "Exact-Head Commit Matching",
            lambda: test_tier3_r3_exact_head_commit_matching(verifier),
        )
        execute_test(
            "T3.3",
            "R3",
            "Tier 3",
            "GitHub Standard Merge Execution",
            lambda: test_tier3_r3_github_standard_merge_protocol(verifier),
        )
        execute_test(
            "T3.4",
            "R3",
            "Tier 3",
            "Remote Branch Deletion Verification",
            lambda: test_tier3_r3_remote_branch_deletion(verifier),
        )
        execute_test(
            "T3.5",
            "R3",
            "Tier 3",
            "Cryptographic Scoped Receipt Minting",
            lambda: test_tier3_r3_scoped_receipt_minting_and_hashing(verifier),
        )
    ...
    if args.tier in ("4", "all"):
        ...
        execute_test(
            ...
            "R4",
            "Tier 4",
            "Worktree Lifecycle & Reaping",
            lambda: execute_temporary(test_tier4_r4_worktree_lifecycle_and_reaping),
        )
        execute_test(
            "T4.2",
            "R4",
            ...
            lambda: test_tier4_r4_zero_unceremonious_closures(verifier),
        )
        execute_test(
            "T4.3",
            "R4",
            "Tier 4",
            "Diagnostic Assessment Retention",
            lambda: test_tier4_r4_diagnostic_assessment_retention(verifier),
        )
        execute_test(
            "T4.4",
            "R4",
            "Tier 4",
            "Batch Remediation Fault Isolation",
            lambda: test_tier4_r4_batch_remediation_fault_isolation(verifier),
        )
        execute_test(
            "T4.5",
            "R4",
            "Tier 4",
            "Closeout Hygiene Predicate",
            lambda: test_tier4_r4_closeout_hygiene_predicate(verifier),
        )
    ...
    if args.invariants or args.tier == "all":
        ...
        execute_test(
            ...
            "R5",
            "Invariant",
            "Private Document Review Untouched",
            lambda: test_invariant_r5_private_document_review_untouched(verifier),
        )
        execute_test(
            "INV.2",
            "R5",
            "Invariant",
            "Conduct Role Hardening Branch Untouched",
            lambda: test_invariant_r5_role_hardening_branch_untouched(verifier),
        )
        execute_test(
            "INV.3",
            "R5",
            "Invariant",
            "Universal Mail Automation Untouched",
            lambda: test_invariant_r5_universal_mail_automation_untouched(verifier),
        )
        execute_test(
            "INV.4",
            "R5",
            "Invariant",
            "Active Codex/OpenCode Daemons Preserved",
            lambda: test_invariant_r5_active_daemons_preserved(verifier),
        )
        execute_test(
            "INV.5",
            "R5",
            "Invariant",
            "Path Traversal & Alias Resistance",
            lambda: test_invariant_r5_path_traversal_and_alias_interception(verifier),
        )
    ...
    live_verified = not args.live or live_res.get("verified") is True
    ...
    if args.json or args.output:
        report_data = {
            "schema": "limen.local_model_verification_report.v1",
            "evidence_scope": "local_git_fixtures_and_synthetic_models",
            ...
            "summary": {
                ...
                "status": "MODEL_TESTS_PASS" if failed_tests == 0 else "FAIL",
            },
    ...
    return 0 if failed_tests == 0 and live_verified else (3 if not live_verified else 1)
