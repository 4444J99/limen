...
def main(argv=None):
    ...
    if args.status:
        ...
        return 0
    canary_mode = "recording" if args.recording_canary else "macos" if args.macos_canary else None
    if args.dry_run and (canary_mode or args.confirm_macos_canary):
        print(
            json.dumps(
                {
                    "apply_required": True,
                    ...
                    "confirm_event_id": args.confirm_macos_canary,
                },
                indent=2,
                sort_keys=True,
            )
    ...
    delivery_failed = any(result.status == "delivery_failed" for result in results) or any(
