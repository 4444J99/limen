...
def validate_contract(data: dict[str, Any]) -> list[str]:
    ...
    _require(
        errors,
        contract.get("status") == "p03_reader_gate_p04_staged",
        "contract must record the P03 reader gate and staged P04 state",
    )
    ...
    _require(
        errors, dependency.get("state") == "reader_gate_open", "formal dependency must record the open reader gate"
    )
    ...
    _require(
        errors,
        accepted.get("p04") == {"phase_id": "PSP-P04", "state": "staged_dependency_blocked", "dependency": "PSP-P03"},
        "P04 staged dependency binding drifted",
    )
    prohibited = set(contract.get("prohibited_preflight_effects", []))
    for effect in ("public-surface publication", "merge to main", "outbound sending"):
    ...
    _require(
        errors,
        any(str(effect).startswith("P04 leaf or phase receipt") for effect in prohibited),
        "missing P04 dependency prohibition",
    )
...
def render_contract(data: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "> **Staged contract — one reader gate remains.** PSP-P02 and PSP-P03-W01 through W06 are accepted. PSP-P03-W07 still requires five genuine independent target-like readers, so PSP-P03 and PSP-P04 remain open. This document does not publish an offer, set a fee, grant authority, or authorize a merge, send, signature, or engagement.",
        "",
        "## Identity contract",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            "Until PSP-P03-W07 and the named commercial gates are satisfied, this remains a staged contract, not a public offer or agreement.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    if args.check:
        ...
        if errors:
            ...
            return 1
        print(
            "PASS: PSP-C03 contract binds accepted P02/P03 evidence, the open W07 reader gate, and staged bounded P04 offers"
        )
    return 0
