...
def _paths() -> tuple[Path, Path]:
    runtime = Path(os.environ.get("LIMEN_IMMUTABLE_RUNTIME", Path.home() / ".local/share/limen/current")).expanduser()
...
def expected_block() -> str:
    ...
    credential_file = Path(os.environ.get("LIMEN_CONDUCT_ENV_FILE", Path.home() / ".limen.env")).expanduser()
    session_id = os.environ.get("LIMEN_SESSION_ID", "")
    command = " ".join(
        [
            f"PATH={shlex.quote(path)}",
            f"LIMEN_ROOT={shlex.quote(str(live_root))}",
            f"LIMEN_NOTIFICATION_STATE_DIR={shlex.quote(str(STATE_ROOT))}",
            f"LIMEN_SESSION_ID={shlex.quote(session_id)}",
...
def _execution_readiness() -> dict[str, object]:
    ...
    try:
        ...
        private_file = False
    session = bool(os.environ.get("LIMEN_SESSION_ID", "").strip())
    return {
        "fresh_authenticated_lease_per_fire": True,
        "session_configured": session,
        "private_credential_file": private_file,
        "configured": session and private_file,
        "broker_admission": "verified_by_each_one_shot_before_effects",
    }
...
def _plan() -> dict[str, object]:
    ...
    return {
        ...
        "schedule": block,
        "runtime": str(runtime),
        "runtime_sha": runtime.resolve().name if len(runtime.resolve().name) == 40 else None,
...
def _apply(expected_digest: str) -> int:
    ...
    if not plan["execution_readiness"]["configured"]:
        raise ValueError(
            "notification schedule requires its registered LIMEN_SESSION_ID and private conduct environment file"
        )
...
def _status() -> int:
    ...
    runtime, _ = _paths()
    installed = current.count(BEGIN) == 1 and current.count(END) == 1 and current.count(block) == 1
    ...
    payload = {
        ...
        "one_shot": one_shot,
        "managed_block_count": current.count(BEGIN),
        "execution_readiness": readiness,
        "complete": installed and executable and one_shot and readiness["configured"],
    }
