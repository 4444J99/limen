...
DEFAULT_PRIOR_OBSERVATION = ROOT / "docs" / "continuations" / "universe-recovery-20260823" / "stable-observation.json"
...
def main() -> int:
    ...
    parser.add_argument("--prior-observation", type=Path, default=DEFAULT_PRIOR_OBSERVATION)
    ...
    try:
        ...
        if result.ok:
            prior = RecoveryStableObservationV1.model_validate_json(args.prior_observation.read_text(encoding="utf-8"))
            if prior.stable_digest != result.stable_digest:
                raise ValueError("prior recovery observation has a different stable digest")
            if prior.observed_at >= manifest.generated_at:
                raise ValueError("prior recovery observation must predate the current manifest")
