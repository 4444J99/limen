...
def canonical_owners():
    ...
    if not isinstance(estate, dict) or not estate.get("schema_version") or not estate.get("classes"):
        raise RuntimeError("canonical owner registry is unavailable or invalid")
    return gitvs.owners(estate)
...
def main() -> int:
    ...
    except Exception as exc:  # bounded failure: retain the prior cache but fail this producer receipt
