...
def build_view():
    ...
    ships_total, ships_by_repo, recent_refs = read_ships_24h(ROOT)
    ...
    your_gates = [p for p in products if p.get("whose_hand") == "yours" and p.get("stage") in ("deploy-ready", "live")]

    fleet = {
        name: {"health": info.get("health"), "remaining": info.get("remaining")}
...
_STAGE_COLOR = {"building": "#8a93a6", "deploy-ready": "#2ecc71", "live": "#27ae60", "monetized": "#f1c40f"}
_HEALTH_DOT = {
    "ok": "#2ecc71",
    "throttle": "#f1c40f",
    "low": "#e67e22",
...
def render_html(v):
    ...
    for p in v["products"]:
        ...
        hand_badge = (
            '<span class="hand you">YOUR GATE</span>' if hand == "yours" else '<span class="hand me">fleet</span>'
        )
    ...
    if v["your_gates"]:
        items = "".join(
            f"<li><b>{_esc(g.get('product'))}</b> is "
...
def main():
    ...
    for d in OUT_DIRS:
        try:
            ...
            continue
    print(
        f"money-view: {len(view['products'])} products, "
