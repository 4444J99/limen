...
def render(d, ticks, usage=None):
    ...
    for a in [x for x in LANE_ORDER if x in caps] + [x for x in caps if x not in LANE_ORDER]:
        ...
        p(
            f"  {a:9} {bar(pct)} {sp:>3}/{cap:<3} {C['gray']}{rem:>3} left · {by_lane_open.get(a, 0):>2} queued{C['x']}  {kind}{C['x']}"
        )
    ...
    if integ:
        ...
        if nch:
            p(
                f"  {C['m']}⚑ chronic:{nch}{C['x']} {C['gray']}(reopened ≥3× · never a PR · failing all lanes → escalate, not re-loop){C['x']}"
            )
    ...
    if ticks:
        ...
        last = ticks[-1]
        p(
            f"  {C['gray']}last {last.get('ts', '?')} · spent {last.get('daily_spent', 0)}/{last.get('daily_cap', 0)}{C['x']}"
        )
    ...
    if usage and usage.get("vendors"):
        ...
        for a, v in usage["vendors"].items():
            ...
            if poss:
                ...
                used_pct = 100 - hp
                p(
                    f"  {a:9} {bar(used_pct)} {C['w']}{cons}/{fmt_n(poss)}{C['x']} {v.get('unit', '')[:3]:3} {C['gray']}{hp}% left{C['x']} {hcol}{h}{C['x']}"
                )
            else:
                p(
                    f"  {a:9} {C['gray']}{'░' * 22}{C['x']} {C['w']}{cons}{C['x']} {v.get('unit', '')[:3]:3} {C['gray']}no cap{C['x']} {hcol}{h}{C['x']}"
                )
    ...
    if events:
        ...
        for e in events:
            ec = (
                C["g"]
                if ("PR" in e or "dispatched" in e)
                else C["y"]
                if ("RATE" in e or "reopened" in e)
                else C["gray"]
            )
            p(f"  {ec}{e}{C['x']}")
